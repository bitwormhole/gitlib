...
func (inst *TestListObjectsInPack) run(task *cli.Task) error {
	...
	for _, pid := range plist {
		err := inst.logObjectsInPack(pid, session)
		if err != nil {
			return err
		}
	}
...
func (inst *TestListObjectsInPack) logObjectsInPack(pid git.PackID, session store.Session) error {
...
func (inst *TestListObjectsInPack) parseDeltaRawContent(item *tagListObjectsInPackItem, session store.Session) error {
